...
def web():
    ...
    if st.button('Scrape'):
        ...
        st.write(text_from_html_llm(response.text, model=model))

    if st.button('Summarize'):
        response = requests.get(input_url)
        st.write(summarize_article(text_from_html(response.text), model=model))

    question_box = st.text_input("Ask a question here!")
    if st.button('Answer') and question_box:
        response = requests.get(input_url)
        st.write(answer_question(text_from_html(response.text), question_box, model=model))